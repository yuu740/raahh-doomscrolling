...
frame_count = cap_alarm.get(cv2.CAP_PROP_FRAME_COUNT)
...
print("Aplikasi Berjalan... Pantau terus!")
...
while True:
    ...
    if is_alarm_active:
        ...
        sync_pos_ms = audio_pos_ms % duration_ms
        
        cap_alarm.set(cv2.CAP_PROP_POS_MSEC, sync_pos_ms)
        
        ret_a, frame_a = cap_alarm.read()
        if ret_a:
            h, w, _ = frame.shape
            frame_a = cv2.resize(frame_a, (int(w*0.8), int(h*0.8)))
            ah, aw, _ = frame_a.shape
            ...
            cv2.putText(frame, "GET BACK TO WORK!", (x_offset+20, y_offset-20), 
                        cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 255), 2)

    else:
        cv2.putText(frame, "SCANNING ACTIVE", (50, 40), 
    ...
    cv2.imshow("Doomscroll Blocker v2.5", frame)
